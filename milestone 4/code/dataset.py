...
class dataset:
    def __init__(self, FNC_PATH="./dataset/FNC-1", LIAR_PATH="./dataset/LIAR",
                 word2vec=True) -> None:
        self.FNC_PATH = FNC_PATH
        self.LIAR_PATH = LIAR_PATH

        FNC_train, FNC_val, FNC_test = self.load_dataset(self.FNC_PATH)
        LIAR_train, LIAR_val, LIAR_test = self.load_dataset(self.LIAR_PATH,
                                                            header=None)
    ...
    def show_dataset_info(train_df, val_df, test_df, dataset_name: str):
        ...
        for dataset, name in zip([train_df, val_df, test_df], dataset_name):
            print(
                f"\t{name:10s} has {dataset.shape[0]:5d} rows and shapes {dataset.shape}.")
    ...
    # split the merged df 
    @staticmethod
    def split_merged_dataset(merged_dataset: pd.DataFrame,
                             length_list: List[int]) -> List[pd.DataFrame]:
        assert sum(length_list) == merged_dataset.shape[
            0], "incorrect length_list"
    ...
    def get_word2vec(sentence):
        ...
        word_vectors = [word2vec_model[word] for word in tokenized_sentence if
    ...
    def preprocess_FNC(self) -> None:
        ...
        merged_FNC_df['X'] = merged_FNC_df["Headline"] + merged_FNC_df[
            "articleBody"]

        if self.word2vec:
            merged_FNC_df = merged_FNC_df.reset_index(drop=True)
            merged_FNC_df["statement_wv"] = merged_FNC_df["X"].apply(
                self.get_word2vec)

            # split a column of list to 300 columns
            temp_df = pd.DataFrame(merged_FNC_df["statement_wv"].to_list(),
                                   columns=[f"vector_{i + 1}" for i in
                                            range(300)])
            merged_FNC_df = pd.concat([merged_FNC_df, temp_df], axis=1)

            self.FNC_data = self.split_merged_dataset(merged_FNC_df,
                                                      length_list)
    ...
    def preprocess_LIAR(self) -> None:
        ...
        column_names = ["statement ID", "label", "statement", "subject(s)",
                        ...
                        "barely true", "false", "half true", "mostly true",
                        "pants on fire",
                        "context(location)"]
        ...
        merged_LIAR_df['label'] = merged_LIAR_df['label'].map(
            label_binary_liar)

        # subject column -> one-hot encoding
        merged_LIAR_df["subject(s)"] = merged_LIAR_df["subject(s)"].replace(
            np.nan, '')

        subject = merged_LIAR_df["subject(s)"].replace(np.nan,
                                                       '')  # There is nan in the column
        unique_subject = Counter(
            [j for i in subject for j in i.split(",") if j])
        ...
        for subject in unique_subject.keys():
            merged_LIAR_df[f"subject_{subject}"] = merged_LIAR_df[
                "subject(s)"].apply(lambda x: subject in x)

            # assign subject_other to True for empty subject or subject count less than `subject_thredhold`
        merged_LIAR_df['subject_other'] = merged_LIAR_df[[i for i in
                                                          merged_LIAR_df.columns
                                                          if
                                                          "subject_" in i]].sum(
            axis=1) <= 0
        ...
        label_count = merged_LIAR_df[["label", "state"]].groupby(
            "state").count().sort_values(by="label")
        filtered_state = label_count[
            label_count["label"] >= state_thredhold].index
        merged_LIAR_df["raw_state"] = merged_LIAR_df["state"]
        merged_LIAR_df["state"] = np.where(
            merged_LIAR_df["state"].isin(filtered_state),
            merged_LIAR_df["state"], "other")

        merged_LIAR_df = pd.concat(
            [merged_LIAR_df, pd.get_dummies(merged_LIAR_df["state"])], axis=1)

        merged_LIAR_df['party_other'] = ~merged_LIAR_df[
            "party affiliation"].isin(["democrat", "republican"])
        merged_LIAR_df['party_democrat'] = merged_LIAR_df[
                                               "party affiliation"] == "democrat"
        merged_LIAR_df['party_republican'] = merged_LIAR_df[
                                                 "party affiliation"] == "republican"

        drop_columns = ["statement ID", "subject(s)", "speaker",
                        "speaker's job title", "state", "raw_state",
                        "barely true", "false", "half true", "mostly true",
                        "pants on fire", "party affiliation"]
        ...
        if self.word2vec:
            ...
            merged_LIAR_df["statement_wv"] = merged_LIAR_df["statement"].apply(
                self.get_word2vec)

            # split a column of list to 300 columns
            temp_df = pd.DataFrame(merged_LIAR_df["statement_wv"].to_list(),
                                   columns=[f"vector_{i + 1}" for i in
                                            range(300)])
    ...
    def __call__(self, dataset: str = "LIAR", type: str = "train",
                 all: bool = False):
        type_mapping = {
            "train": 0,
            "val": 1,
