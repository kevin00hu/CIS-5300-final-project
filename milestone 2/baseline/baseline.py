...
if __name__ == "__main__":
    ...
    train = torch.utils.data.TensorDataset(torch.tensor(train_X.values),
    ...
    for epoch in range(epoches):
        ...
        with torch.no_grad():
            ...
            val_accuracy = accuracy_score(val_y.cpu(),
                                          np.argmax(pred.cpu().numpy(),
                                                    axis=1))
            print(
    ...
    with torch.no_grad():
        ...
        test_y, test_pred = test_y.cpu(), np.argmax(test_pred.cpu().numpy(),
                                                    axis=1)
    ...
    plt.plot(range(len(val_loss_records)), val_loss_records,
             label="validation")
    plt.xlabel('epoch')
    plt.ylabel('loss')
